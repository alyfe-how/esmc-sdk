 ...
 * Usage: node e7f6cde4.js --mode=full_deployment --turn=N --request=M [--silent]
 *
 * Architecture:
...
if (!mode) {
  if (!isSilent) {
    console.error(JSON.stringify({
      ...
      usage: "node e7f6cde4.js --mode=<full_deployment|lightweight> --turn=N --request=M [--silent]"
    }, null, 2));
  }
  process.exit(1);
}
...
if (!fs.existsSync(cifsPath)) {
  if (!isSilent) {
    console.error(JSON.stringify({
      ...
      instruction: "Execute: node db8a0273.js check A [SUB_TOTAL] [current_turn] first"
    }, null, 2));
  }
  process.exit(1);
}
