 ...
 * ════════════════════════════════════════════════════════════════
 */
/** ESMC 3.101.0 (Silent-by-default) - Base: 3.52 Profile Synthesis CLI */
...
function showUsage() {
    console.log(`
🎖️ Profile Synthesis CLI - Synthesize user profiles from multiple sources

Usage: node c89b6bd7.js <command> [options]

Commands:
  synthesize               Synthesize user profile
  merge <profiles>         Merge multiple profiles
  help                     Show this help

Version: ESMC 3.52 | Module: 0b43cae8.js
    `);
}
...
async function main() {
    ...
    try {
        if (command === 'help' || command === '--help') { if (silent) {
            ...
            showUsage(); return; }
        const Module = require('./0b43cae8.js');
