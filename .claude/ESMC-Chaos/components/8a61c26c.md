...
**Purpose:** Route AEGIS operations → Lazy-load modular components
...
**⚠️ IMPORTANT: This manifest should ONLY be loaded via 6c390b10.md routing**
- If you're reading this directly (without loading 6c390b10.md first) = PROTOCOL VIOLATION
- Correct flow: User request → 6c390b10.md → 8a61c26c.md → AEGIS Component Router
- BOOTSTRAP handles: routing, tier detection, version checks, manifest loading

---

## ⚡ EXECUTION PROTOCOL: AEGIS COMPONENT ROUTER

**Philosophy:** Detect operation → Route → Load ONLY the needed module (BOOTSTRAP pattern applied to AEGIS)
...
- `"write document"` OR `"write brief"` OR `"create document"` → OPERATION = write
- `"changelog"` OR `"CL tag"` OR `"insert CL"` → OPERATION = cl

**Precedence 3 (Maintenance Operations):**
- `"garbage collect"` OR `"clean memory"` OR `"gc memory"` → OPERATION = gc
- `"audit"` OR `"execution log"` → OPERATION = audit
...
- If OPERATION = seed: `"🧠 AEGIS: 'seed' detected → Executing seed-session.js"`
- If OPERATION = echo: `"📸 AEGIS: 'echo' detected → DEPRECATED (use seed workflow)"`
- If OPERATION = write: `"📝 AEGIS: 'write' detected → Loading aegis-write.js"`
- If OPERATION = cl: `"📋 AEGIS: 'changelog' detected → Executing CL protocol"`
- If OPERATION = gc: `"🗑️ AEGIS: 'gc' detected → Executing GC protocol"`
- If OPERATION = audit: `"📊 AEGIS: 'audit' detected → Loading aegis-audit.js"`
...
**⚠️ EXECUTION COMMAND (Use Bash tool for Step 3 only):**
```bash
node ".claude/ESMC Complete/core/seed-session.js"
```

**🆕 ESMC 3.98.1: Compressed Seed Protocol (Token-Optimized)**
...
**Execute SEED Protocol (Steps 1-3 in detail):**
   ...
   **⚠️ CRITICAL: seed-session.js automatically finds the most recent session file**
...
**⚠️ ECHO DEPRECATED (ESMC 3.80+):** Use seed workflow instead.

**Legacy Support (for ESMC < 3.80):**

Use Bash tool:
```bash
node ".claude/ESMC-Chaos/components/bd34357a.js" increment
```

**STOP.** ECHO is a terminal operation.

**📋 Legacy ECHO Documentation:** `.claude/memory/documents/reference/ECHO-ARCHITECTURE.md`

---

#### **If OPERATION = write:**

**1. Use Read tool:** `.claude/ESMC-Chaos/components/b1ed547a.js`
...
**1. Use Bash tool:** `node .claude/ESMC-Chaos/components/1ed50eab.js <command>`
...
**1. Use Bash tool:** `node .claude/ESMC-Chaos/components/8c12d6af.js <command>`
...
**1. Use Read tool:** `.claude/ESMC-Chaos/components/b7ff15ca.js`
