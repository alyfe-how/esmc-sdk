...
**Purpose:** Lazy-load protocol for tasks with ≥50 line changes
**Token Cost:** ~3,189 tokens
**Load Trigger:** Changes ≥50 lines OR complexity ≥40 OR user requests changelog

---

<!-- CL5 - ESMC 3.29 Atomic CL Workflow Enforcement Protocol -->
<!-- EXECUTION PROTOCOL -->
## 🆕 ESMC 3.29 - ATOMIC CL WORKFLOW EXECUTION PROTOCOL

**⚠️ MANDATORY PROTOCOL - APPLIES TO ALL CODE CHANGES ≥50 LINES**

**🎯 UNIVERSAL RULE:** Embed CL tags DURING presentation, execute writes atomically AFTER user approval
...
### **⚠️ YOU MUST EXECUTE THESE STEPS WHEN CHANGES ≥50 LINES:**
...
**2. Use Glob tool (MANDATORY - Execute NOW):**
   ...
   - Example: Glob finds `BRAIN-CL4.md` (count=1) → Next is CL5 (NOT CL1)

**2.5 🛑 CHECKPOINT: Did you execute Glob tool in step 2?**
   - If NO: Go back and execute Glob NOW
   - If YES: You have correct CL number from count, proceed to step 3

**🚫 DO NOT proceed without executing Glob in step 2 above**
...
**4.5 🛑 CHECKPOINT: Does your path match the constant above?**
...
**🚫 DO NOT proceed with incorrect path (step 4 above)**

**5. 🛑 CHECKPOINT: Did you embed CL tag in presented code?**
   - If NO: Go back to step 3 - DO NOT present code without CL tag
   - If YES: Present complete solution to user (fix + CL tag visible)

**🚫 DO NOT present code without CL tag embedded (step 3 above)**
...
```javascript
const { AEGISSystem } = require('./.claude/ESMC-Chaos/components/babd175c.js');
...
**📋 Complete CL Protocol Documentation:** `.claude/memory/documents/colonels/CL-PROTOCOL.md`
