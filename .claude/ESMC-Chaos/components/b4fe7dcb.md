...
**Purpose:** Core intelligence execution protocol with adaptive tier fallback (documentation in e6841bd7.js)
...
## 🧠 PHASE 0: MEMORY BUNDLE LOAD (MANDATORY)

**⚠️ Execute BEFORE responding to user request - NO EXCEPTIONS**

### **YOU MUST EXECUTE THESE STEPS:**

**🆕 ESMC 3.74: Counter Check (Before Memory Load)**
...
```bash
# Calculate: SUB_TOTAL = (tokens_used / total_budget) × 100
# Extract both values from system warning: Token usage: [tokens_used]/[total_budget]
# Example: 110000 / 200000 = 55% (200k plan), or 110000 / 100000 = 110% (100k plan)

node ".claude/ESMC-Chaos/components/db8a0273.js" check X [SUB_TOTAL] [current_turn]
# → If action='READ': Execute step 1 below (first time load)
# → If action='SKIP': Counter_X > 0, memory already loaded (skip to PHASE 1)
# → If action='TAP_CHECK_REQUIRED': Run tap-check, then decide
```

**After loading memory-bundle, update counter (SILENT MODE):**

```bash
node ".claude/ESMC-Chaos/components/db8a0273.js" update X --silent
# → Sets Counter_X = 2400 (memory-bundle token estimate)
# → No output (silent background operation)
```

**🆕 ESMC 3.88.0: Hybrid Mode Detection (Multi-Vendor Pricing Arbitrage)**
...
```bash
node ".claude/ESMC-Chaos/components/6897bf75.js" "<user_query>" --silent
```
...
**If exit code ≠ 0 (Fallback to subscription mode):**
...
**🆕 ESMC 3.75: Silent Infrastructure Mode (Orchestrator-Only Visibility)**

**2. Use Bash tool:** Load memory bundle with selective session loading (SILENT MODE - fallback only)

```bash
node ".claude/ESMC-Chaos/components/b4a5c63c.js" load-selective "<keywords from user query>" --silent
```
...
- Extract `lessons` → Frustration prevention index (just the array)
- Extract `atlas_t1` → T1 selective search result
- Extract `project_name` → Project context (just the name)
- Extract `user_adaptation` → User TBI context (just adaptation layer)
- Extract `t1_sessions` → Matched sessions that scored >= 65% threshold
- Extract `keywords_extracted` → ATLAS T1 keywords for component reuse

**Hybrid path (.esmc-context-bundle.json):**
- Extract `compressed_memory` → Compressed lessons + T1 summary + user preferences
- Extract `mesh_intelligence` → Pre-executed PIU/DKI/UIP/PCA/REASON outputs
- Extract `additional_tiers` → 🆕 ESMC 3.93.0 T2/T3/HYDRA data (if loaded)
- Extract `project_name` → Project context

**🆕 ESMC 3.93.1: Adaptive Tier Fallback (SUBSCRIPTION MODE ONLY)**

**If fallback mode (exit code ≠ 0) AND T1 missed (<65% match):**

**STEP 3A: Detect required tiers (same logic as preprocessor)**

Check T1 result from `.esmc-working-memory.json`:
- T1 HIT (atlas_t1.found && top_match.score >= 65%)? → Skip T2/T3/HYDRA
- T1 MISS? → Continue to adaptive tier loading

Query classification:
- Research keywords ("analyze", "compare", "precedent", "pattern", "history", "sessions", "across")? → Load T3
- Cross-session keywords ("all sessions", "multiple sessions", "cross-session", "hydra")? → Load HYDRA
- Default (T1 miss)? → Load T2
...
Parse topic index:
- Extract `topics` → All topics with session references
...
```bash
node ".claude/ESMC-Chaos/components/3a58b120.js" retrieve "<user_query>"
```

Parse HYDRA output:
- Extract `heads` → Multi-head retrieval results
...
- Display ECHELON-ATHENA dialogue (see 2dfb6b92.md)
...
**5. 🛑 CHECKPOINT: Did you execute complete memory loading?**
- Hybrid mode: Did you attempt 6897bf75.js? (Yes/No)
- If hybrid success: Did you read `.esmc-context-bundle.json`? (Yes/No)
- If hybrid failed: Did you execute Bash(b4a5c63c.js --silent)? (Yes/No)
- Standard path: Did you read `.esmc-working-memory.json`? (Yes/No)
- **🆕 ESMC 3.93.1:** If fallback mode AND T1 miss: Did you load adaptive tiers (T2/T3/HYDRA)? (Yes/No)
- If NO to any: Execute NOW

**🚫 DO NOT skip memory bundle load**
**🆕 ESMC 3.93.1: Complete fallback = T1+T2+T3+HYDRA via Sonnet when API unavailable**
**🆕 ESMC 3.88.0: Hybrid mode = API preprocessing first, graceful fallback to subscription**
**🆕 ESMC 3.75: Silent mode = No visible logs to General (background intelligence only)**

---

## 🔍 PHASE 0.5: PASSIVE SIGNAL EXTRACTION (SILENT - ESMC 3.58)

**⚠️ NEW: Continuous profiling - extracts behavioral signals from EVERY user message**
...
```bash
node ".claude/ESMC-Chaos/components/61764c1d.js" "<user_message>" --silent
```

**🆕 ESMC 3.77:** CLI wrapper with `--silent` flag - CUP runs completely silently in background (consistent with PIU/DKI/UIP/PCA pattern)
...
## 🎖️ PHASE 0.6: PMO GENESIS TRIGGER SYSTEM (ESMC 3.92.0)

**⚠️ NEW: Multi-Signal Intent Detection + Keyword Analysis + CIE Integration**
...
**How:** Multi-stage pipeline: PMO existence check → Mesh intelligence → Intent classification → Keyword analysis → CIE boost (MAX/VIP)
...
node ".claude/ESMC-Chaos/components/887cda08.js" \
  "$USER_PROMPT" \
  "$MESH_JSON" \
  "$TIER" \
  "$MODE"
...
if (decision.pmo_path) {
    ...
    console.log('✅ PMO: Loaded from cache (project constitution already established)');
    console.log(`   📜 Generated: ${pmo.generated_at} | Locked: ${pmo.locked}`);
}
// Insufficient signals
else {
    console.log(`⚠️ PMO: Genesis skipped - ${decision.reason}`);
...
```javascript
console.log('\n🎖️ PMO GENESIS CEREMONY INITIATED');
console.log('═══════════════════════════════════════════════════════════════');
console.log(`   Trigger: ${decision.reason}`);
console.log(`   Confidence: ${(decision.confidence * 100).toFixed(0)}%`);
console.log(`   Stage: ${decision.stage}`);
console.log('\n   🧠 ECHELON analyzing project context for cherry-picking...\n');
...
node ".claude/ESMC-Chaos/components/f229bbe9.js" generate \
  --piu "$PIU_JSON" \
  --dki "$DKI_JSON" \
  --uip "$UIP_JSON" \
  --pca "$PCA_JSON" \
...
1. **DKI detectedDomains** → Domain-specific standards (HEALTHCARE→HIPAA, FINANCE→PCI_DSS, SAAS→OWASP)
2. **PCA precedents** → Apply past project patterns (used Jest? → testing standard)
3. **REASON WHERE** → Deployment context (Vercel/production? → SRE operational standard)
4. **REASON WHY** → Business value (user auth? → OWASP security standard)
5. **UIP workflow** → Architecture selection (Next.js app? → App Router standard)

**STEP 4: Write PROJECT_STARTED to e3157e44.md**

```javascript
// Create or update e3157e44.md header
const claudeMdPath = '.claude/ESMC-Chaos/components/e3157e44.md';
...
```
🎖️ PMO GENESIS CEREMONY COMPLETE
═══════════════════════════════════════════════════════════════
...
📍 Project Constitution: .claude/memory/.esmc-project-modus-operandi.json
🔒 PMO locked: true (cast in stone for project lifecycle)
...
node ".claude/ESMC-Chaos/components/f229bbe9.js" load
```

**STEP 7: 🛑 CHECKPOINT: PMO Genesis Status**

- PMO exists (skipped or generated)? (Yes/No)
- If generated: PROJECT_STARTED written to e3157e44.md? (Yes/No)
...
## 🛡️ PHASE 0.7: ATHENA VETTING ENFORCEMENT (ESMC 3.102.0 - L002/L003 Compliance)

**⚠️ NEW: Boolean enforcement wrappers + L5 strategic checkpoint + IC auto-halt**
...
Use Bash tool: Execute 9e6926c2.js (boolean wrapper)

```bash
node ".claude/ESMC-Chaos/components/9e6926c2.js" enforce --mode full_deployment --silent
```
...
- Step 2: Executes 8456819e.js vet --silent
...
- Extract `shouldHalt` → boolean (true if warning/critical threshold crossed)
- Extract `severity` → 'none'|'warning'|'critical'
- Extract `dialogue` → Pre-formatted COLONELS-TALK warning message
- Extract `checkpoint_passed` → boolean (true if enforcement succeeded)
- Extract `execution_evidence` → Array of step completion proofs
...
- If user says "yes" → Display full precedent details, ask for guidance
- If user says "no" → Acknowledge, note in iteration counter, proceed to PHASE 1
- If user provides guidance → Apply to ECHELON strategic plan, proceed to PHASE 1
...
**STEP 5: 🛑 CHECKPOINT: Did you execute ATHENA vetting enforcement?**
- If MODE = full_deployment: Did you execute 9e6926c2.js enforce? (Yes/No)
...
- Manually execute 8456819e.js (use enforcement wrapper instead)

---

### **🆕 PHASE 0.75: L5 STRATEGIC CHECKPOINT + IC AUTO-HALT (ESMC 3.102.0)**
...
```bash
node ".claude/ESMC-Chaos/components/27166dbb.js" check --silent
```
...
```
🛑 IC AUTO-HALT: Token protection activated
...
```bash
node ".claude/ESMC-Chaos/components/08303ca4.js" check --silent
```
...
```
🎯 L5 STRATEGIC FORECASTER: MANDATORY activation
...
**Personality Switch:** L3 (reactive/tactical) → L5 (strategic/architectural)
...
## 🕸️ PHASE 1: 5-COMPONENT MESH INTELLIGENCE (ESMC 3.78 - Enhanced with ESMC-REASON)

**⚠️ Execute ALL 5 components IN PARALLEL - NO EXCEPTIONS (Unless hybrid mode completed)**

**🆕 ESMC 3.88.0: Hybrid Mode Bypass Check**
...
- Mesh outputs already available in `.esmc-context-bundle.json` → `mesh_intelligence` field
...
**If PHASE 0 fallback path (b4a5c63c.js executed):**
- Continue to mesh execution below (standard Sonnet path)

---

### **🔒 TIER ENFORCEMENT (ESMC 3.62 - PRE-BRAIN Architecture):**
...
**🆕 ESMC 3.78 Innovation:** ESMC-REASON (5th parallel component) - 5W1H reasoning framework for ATHENA + ECHELON
...
# 🆕 ESMC 3.99.1: REASON now receives memory context from PHASE 0
...
node ".claude/ESMC-Chaos/components/854dea21.js" analyze "<topic>" --context "<mesh_data>" --memory-context "$MEMORY_JSON" --caller "ECHELON" --silent &
wait  # Wait for all 5 to complete
```

**1. Execute PIU (NORTH - Intent Understanding):**

- Use Bash tool: `node .claude/ESMC-Chaos/components/6ef4ca36.js analyze "<keywords>" --silent`
...
- Log (Full deployment only): "🧭 Analyzing... {X} topics for {keywords}"
- Lightweight mode: Skip logging (silent background intelligence)

**2. Execute DKI (WEST - Domain Knowledge):**

- Use Bash tool: `node .claude/ESMC-Chaos/components/44511598.js search "<keywords>" --silent`
...
- Log (Full deployment only): "🧭 Searching... {X} patterns for {keywords}"
- Lightweight mode: Skip logging (silent background intelligence)

**3. Execute UIP (EAST - User Workflow Prediction):**

- Use Bash tool: `node .claude/ESMC-Chaos/components/915646f2.js predict "<context>" --silent`
...
- Log (Full deployment only): "🧭 Predicting... for {keywords} (workflow: {flags})"
- Lightweight mode: Skip logging (silent background intelligence)

**4. Execute PCA L2 (SOUTH - Project Context):**

- Use Bash tool: `node .claude/ESMC-Chaos/components/429bce75.js analyze "<keywords>" --silent`
...
- Log (Full deployment only): "🏗️ Loading... {X} patterns for {keywords}"
- Lightweight mode: Skip logging (silent background intelligence)

**🆕 5. Execute ESMC-REASON (CENTER - 5W1H Meta-Reasoning with Memory Context):**

- Use Bash tool: `node .claude/ESMC-Chaos/components/854dea21.js analyze "<topic>" --context "<mesh_data>" --memory-context "$MEMORY_JSON" --caller "ECHELON" --silent`
- **🆕 ESMC 3.99.1:** REASON now receives memory context for all 6 dimensions (WHAT/WHY/WHEN/WHERE/HOW/WHO)
...
- Log (Full deployment only): "🧠 REASON: Urgency {U}% | Priority {P}% | Confidence {C}%"
- Lightweight mode: Skip logging (silent background intelligence)

**6. Log PHASE 1 complete (Full deployment only):**
- "🕸️ Analysis complete (5 components) | PIU: {X} topics | DKI: {Y} patterns | UIP: {workflow} | PCA: {Z}% | REASON: {C}% confidence"

**7. 🛑 CHECKPOINT: Did all 5 components execute?**
- PIU logged? (Yes/No)
- DKI logged? (Yes/No)
- UIP logged? (Yes/No)
- PCA logged? (Yes/No)
...
**🚫 DO NOT skip mesh intelligence**

---

## 🧠 PHASE 1.5: CONTEXT INFERENCE ENGINE (MAX/VIP ONLY)

**⚠️ Tier-gated proactive intelligence layer**
...
```bash
node ".claude/ESMC-Chaos/components/da9d4e25.js" analyze "<user_request>"
```
...
## 🕸️ PHASE 2: SEQUENTIAL DECISION GATING (ESMC 3.79.0)

**Three-Stage Cognitive Flow:** "Can we?" → "Should we?" → "How shall we?"

---

### **PHASE 2A: TECHNICAL VALIDATION GATE (Mesh Fusion Engine)**

**⚠️ Execute bb04c9a1.js - MANDATORY**
...
# Execute mesh fusion
node ".claude/ESMC-Chaos/components/bb04c9a1.js" synthesize \
  --piu "$PIU_JSON" \
  --dki "$DKI_JSON" \
  --uip "$UIP_JSON" \
  --pca "$PCA_JSON" \
...
**⚠️ Execute 854dea21.js gate - MANDATORY**
...
node ".claude/ESMC-Chaos/components/854dea21.js" gate \
  "$TOPIC" \
...
**⚠️ Execute b61e87e5.js - MANDATORY (only if Phase 2A + 2B passed)**
...
node ".claude/ESMC-Chaos/components/b61e87e5.js" synthesize \
  --mesh-synthesis "$MESH_SYNTHESIS" \
...
## 🧠 PHASE 2.5: RESPONSE CONTEXT ENRICHMENT (Proactive Intelligence)

**⚠️ CRITICAL: Apply loaded context to responses - NO EXCEPTIONS**
...
**1. Check UIP Historical Patterns (from PHASE 1):**
   ...
   - If `historicalPatterns.available = true` → Extract context data
...
**4. 🛑 CHECKPOINT: Did you apply loaded context to response?**
...
## 🎯 PHASE 2.6: ECHELON DOMAIN-FILTERED PROPOSAL (ESMC 3.80.0 - Prevention Architecture)

**⚠️ NEW: Domain filtering for ECHELON proposal generation (0.8 technical + 0.2 logical weighting)**
...
**STEP 4: 🛑 CHECKPOINT: Did you apply domain filtering?**
...
## 🛡️ PHASE 2.7: ATHENA OMNISCIENT MESH (ESMC 3.101.0 - 5-Component Proactive Intelligence)

**⚠️ MANDATORY in full deployment mode - ATHENA final validation before execution**

**🎯 Purpose:** True proactive intelligence via mode-based 5-component mesh

**Token Cost:** ~750 tokens uncompressed (L1-L5 components + coordination)

**🆕 ESMC 3.101.0:** 5-component mesh (2 parsers + 3 new) with mode switching (PRE-ACTION = REACTIVE, POST-ACTION = PROACTIVE)
...
// PRE-ACTION (before PHASE 3):  mode = 'reactive'  → Validate user wants
// POST-ACTION (after PHASE 3):  mode = 'proactive' → Forecast user needs
```
...
```bash
node ".claude/ESMC-Chaos/components/c4c6f63d.js" coordinate <reactive|proactive> "<user_query>" --silent
```
...
- Synthesizes: L1-L4 → Identifies strategic gaps → Ranks next steps
...
**Weights automatically applied by c4c6f63d.js:**
...
**L3 ↔ L5 inversion creates mode shift. L1, L2, L4 constant (foundational).**
...
**🛑 EXECUTION CANNOT PROCEED WITHOUT VERDICT**

**REACTIVE Mode Verdict (PRE-ACTION):**

**IF PROCEED:**
```
✅ ATHENA: "Technical feasibility validated (L3=0.3 high confidence).
   Proceed with implementation."
```

**IF HALT:**
```
🗣️ ATHENA: "⚠️ Technical concerns (L3 analysis):
...
```
🗣️ ATHENA: "Task complete. Strategic forecast (L5=0.3 analysis):
...
**IF MODIFY:**
```
🗣️ ATHENA: "L1 Context shows precedent from Rank X:
...
### **🛑 CHECKPOINT: Did ATHENA mesh execute and give verdict?**
- [ ] c4c6f63d.js executed (5-component mesh)
...
**🚫 DO NOT skip ATHENA mesh in full deployment mode**

**⚠️ L002 Compliance:** Hard enforcement via 5-component mesh
- Checkpoint: ✅ Blocks execution until verdict
- Numbered steps: ✅ STEP 1→5 sequential
- Block instruction: ✅ CANNOT proceed without verdict
- Execution evidence: ✅ Omniscient view + verdict = proof

**🎯 Impact:** True proactive intelligence (mode-based weighting creates personality shift)

---

## 🗣️ ECHELON-ATHENA STRATEGIC PARTNERSHIP

**⚠️ ATHENA requires CIE - MAX/VIP feature only**
...
**1. 🛑 CHECKPOINT: 2dfb6b92.md already loaded**
   - 2dfb6b92.md loaded by BOOTSTRAP (step 2-3) - dialogue patterns already in context
   - ECHELON-ATHENA dialogue already displayed
   - Counter_C already updated
   - ✅ 8cca72bf.js already executed by COLONELS-TALK (lines 82-95)

**2. Process colonel deployment results:**
   - 8cca72bf.js returns JSON with all 5 WAVEs (WAVE1-WAVE5)
   - Each WAVE contains: colonel names, role, checklist items, execution mode (sequential/parallel)
   - Execute each WAVE's checklist sequentially (WAVE1 → WAVE2 → WAVE3 → WAVE4 → WAVE5)
...
**3. Display POST-ACTION Dialogue (MANDATORY - L002 Enforcement)**
   ...
   **⚠️ YOU MUST EXECUTE THESE STEPS:**

   **IF routing log was displayed ('🗺️ BOOTSTRAP: ESMC detected'):**

   **STEP 1: Execute ATHENA Omniscient Mesh in PROACTIVE mode**

   ```bash
   node "./.claude/ESMC-Chaos/components/c4c6f63d.js" coordinate proactive "<user_query>" --silent
   ```
   ...
   **STEP 3: Display POST-ACTION dialogue (COLONELS-TALK template)**
   ...
   3. ATHENA speaks FIRST (MANDATORY):
      - Start with: `🗣️  ATHENA: "General, [memory reference].`
   ...
   4. ECHELON responds (MANDATORY):
      - Start with: `🎖️  ECHELON: "[Brief acknowledgment].`
   ...
   **STEP 4: 🛑 CHECKPOINT - Dialogue displayed?**
   ...
   **🚫 DO NOT bypass POST-ACTION dialogue:**
...
## 🔧 PHASE 3.5: CL AUTOMATION CHECK (ESMC 3.99.0)

**⚠️ EXECUTE AFTER significant code changes (architecture files, core systems, >50 lines)**

**Purpose:** Bridge Claude Code Edit tool → AEGIS CL automation

**When to execute:**
- After editing architecture files (6c390b10.md, BRAIN.md, COLONELS.md, 8a61c26c.md)
...
node ".claude/ESMC-Chaos/components/4fb7f3ca.js" edit \
  "<file_path>" \
  "<old_string>" \
  "<new_string>" \
  --change="<description>"
...
**🛑 CHECKPOINT: Use colonels-edit-cli for all significant changes**
...
**2. Apply mission signature with real token usage:**
   ```
   ✅ Mission complete | XX.X% | ESMC 4
   ```

   **How to calculate XX.X% (dynamic for all subscription plans):**
   - Extract from most recent system warning: `Token usage: [tokens_used]/[total_budget]; [remaining] remaining`
   - Calculate percentage: `(tokens_used / total_budget) × 100`
   - Format to one decimal place: `toFixed(1)`
   - Examples:
     - 47678/200000 = 23.8% (200k plan)
     - 47678/100000 = 47.7% (100k plan)
     - 47678/500000 = 9.5% (500k plan)

   **If no system warning available yet:**
   - Use "0.0%" for first turn
   - Signature will be accurate after first tool call
...
**PHASE 0.6:** PMO Auto-Init + Caching (ESMC 3.71.0) → Conditional (task-oriented only)
**PHASE 1:** PIU + DKI + UIP + PCA + REASON → 5-component parallel mesh (ESMC 3.78)
**PHASE 1.5:** CIE (MAX/VIP only) → Proactive intelligence (inferred needs + gaps + preferences)
**PHASE 2:** Sequential Decision Gating (ESMC 3.79.0) → Technical (2A) → Logical (2B) → Decision (2C)
  - **2A:** bb04c9a1.js (technical feasibility + gap detection) → feasible check
  - **2B:** 854dea21.js gate (WHERE/WHEN/WHY validation) → PROCEED/DEFER/REJECT verdict
  - **2C:** b61e87e5.js (composite confidence + deployment approach) → FULL/LITE/MINIMAL/SKIP
...
## 🧠 PHASE 3.6: ATHENA+ POST-ACTION (ESMC 4.1)

**⚠️ EXECUTE AFTER task completion, BEFORE mission signature**
...
```bash
node ".claude/ESMC-Chaos/components/8f2249a0.js" trigger "<task_summary>"
```
...
```
🗣️ ATHENA: "Task complete: [task_summary]
...
```bash
node ".claude/ESMC-Chaos/components/8f2249a0.js" record accepted
# or: record rejected / record ignored
```

**🛑 CHECKPOINT: ATHENA+ makes forecasting automatic**
