 ...
 * ════════════════════════════════════════════════════════════════
 */
/** ESMC 3.101.0 (Silent-by-default) - Base: 3.52 Linguistic Analyzer CLI */
...
function showUsage() {
    console.log(`
🎖️ Linguistic Analyzer CLI - Analyze communication style and patterns

Usage: node 397c9c8a.js <command> [options]

Commands:
  analyze <text>           Analyze linguistic patterns
  profile                  Create linguistic profile
  adapt                    Get adaptation recommendations
  help                     Show this help

Version: ESMC 3.52 | Module: 58b17ed4.js
    `);
}
...
async function main() {
    ...
    try {
        if (command === 'help' || command === '--help') { if (silent) {
            ...
            showUsage(); return; }
        const { LinguisticAnalyzer } = require('./58b17ed4.js');
        ...
        console.log(JSON.stringify(await analyzer.analyze({ text: options.join(' ') }), null, 2));
        } else if (command === 'profile') {
            console.log(JSON.stringify(await analyzer.createProfile(), null, 2));
        } else {
