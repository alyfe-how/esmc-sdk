 ...
 * ════════════════════════════════════════════════════════════════
 */
/** ESMC 3.101.0 (Silent-by-default) - Base: 3.52 rag-context CLI */
...
function showUsage() {
    console.log(`
🎖️ RAG Context Manager CLI - Retrieval-Augmented Generation context management

Usage: node 75536238.js <command> [options]

Commands:
  retrieve <query>         Retrieve relevant context
  manage                   Manage context database
  help                     Show this help

Version: ESMC 3.52 | Module: rag-context-manager.js
    `);
}
...
async function main() {
    ...
    try {
        if (command === 'help' || command === '--help') { if (silent) {
            ...
            showUsage(); return; }
        const Module = require('../rag-context-manager.js');
