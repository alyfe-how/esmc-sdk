 ...
 *
 * Purpose: CLI wrapper for d5969c27.js (ATHENA + ECHELON exclusive)
 ...
 *   node 5395b3af.js analyze "<topic>" --context '{"key": "value"}' --caller "ECHELON" --silent
 *
 *   # Phase 2B (Sequential gating - NEW)
 *   node 5395b3af.js gate "<topic>" --mesh-synthesis '{"score": 75}' --reason5w1h '{"scores": {...}}' --silent
 *
...
const ESMCReason = require('./d5969c27');
...
USAGE:
  node 5395b3af.js analyze "<topic>" [OPTIONS]
...
EXAMPLES:
  ...
  node 5395b3af.js analyze "PMO validator selection" \\
    --context '{"hasAPI": true, "domain": "HEALTHCARE"}' \\
    --caller "ECHELON" --silent

  # ATHENA: Strategic question generation
  node 5395b3af.js analyze "User authentication implementation" \\
...
TOKEN BUDGET:
  ~300 tokens output (JSON only, module runs in subprocess)
    ...
    if (config.command !== 'analyze' && config.command !== 'gate') {
        ...
        console.error('💡 Run "node 5395b3af.js help" for usage');
        process.exit(1);
    }
    ...
    if (!config.topic) {
        ...
        console.error('💡 Usage: node 5395b3af.js analyze|gate "<topic>" [OPTIONS]');
        process.exit(1);
    }
