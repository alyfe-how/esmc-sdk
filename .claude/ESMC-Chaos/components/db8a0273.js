 ...
 *
 * Usage:
 *   node db8a0273.js check A 45 25
...
function verifyRuntimeIntegrity() {
  ...
  try {
    ...
    const criticalFiles = [
      ...
      'e3157e44.md',                    // Entry point - controls everything
      'db8a0273.js',     // First execution - license + integrity
      '6c390b10.md',                 // Routing logic - manifest loading

      // Layer 2: Authentication & Authorization
      'ee1bb133.js',             // Tier validation
      '0b1c5e26.js',      // License validation

      // Layer 3: Core Manifests (prevent malicious code injection)
      'b4fe7dcb.md',                // Main execution protocol
      '2dfb6b92.md',             // ATHENA dialogue (MAX/VIP)
      '8a61c26c.md'                  // AEGIS seed/echo operations
    ];

    const tamperedFiles = [];

    for (const file of criticalFiles) {
      // Special case: e3157e44.md is in SDK root (not obfuscated)
      let manifestEntry;
      if (file === 'e3157e44.md') {
        manifestEntry = 'e3157e44.md';
      } else {
...
const MANIFESTS = {
  X: {
    name: 'Memory-Bundle',
    path: '.claude/ESMC-Chaos/components/b4a5c63c.js',
    type: 'CLI',
    lines: null,
    tokens: 2400
  },
  A: {
    name: 'BOOTSTRAP',
    path: '.claude/ESMC-Chaos/components/6c390b10.md',
    type: 'MANIFEST',
    lines: 464,
    tokens: 4772
  },
  B: {
    name: 'BRAIN-CORE',
    path: '.claude/ESMC-Chaos/components/b4fe7dcb.md',
    type: 'MANIFEST',
    lines: 694,
    tokens: 1735
  },
  C: {
    name: 'COLONELS-TALK',
    path: '.claude/ESMC-Chaos/components/2dfb6b92.md',
    type: 'MANIFEST',
    lines: 363,
    tokens: 900
  },
  D: {
    name: 'REFERENCE',
    path: '.claude/ESMC-Chaos/components/8a61c26c.md',
    type: 'MANIFEST',
    lines: 406,
    tokens: 7000
  },
...
function shouldRefreshManifest(manifest, currentTurn, subTotal) {
  ...
  // ═══════════════════════════════════════════════════════════
  //
  // CRITICAL: 6c390b10.md must be loaded before ANY workflow
  // If counter A = 0, auto-load 6c390b10.md immediately
  //
  // Why: BOOTSTRAP contains routing logic for all manifests
  //      Without BOOTSTRAP, all subsequent routing is invalid
  //      This makes e3157e44.md enforcement automatic and foolproof
  //
  ...
  if (data.counters.A === 0) {
    ...
    // Read 6c390b10.md and output with line numbers
    ...
    try {
      ...
      console.error(`❌ ERROR: Failed to auto-load 6c390b10.md: ${error.message}`);
...
if (command === 'check') {
  ...
  // Usage: node db8a0273.js check A 45 25
  ...
  // Usage: node db8a0273.js evaluate A --answers '["ans1", "ans2", "ans3", "ans4"]' --lines '[69,214,354,589]'
  ...
  if (answersIndex === -1 || !args[answersIndex + 1]) {
    console.log(JSON.stringify({
      ...
      usage: 'node db8a0273.js evaluate <manifest> --answers \'["answer1", "answer2", "answer3", "answer4"]\' --lines \'[line1,line2,line3,line4]\''
    }, null, 2));
    process.exit(1);
  }
  ...
  // Usage: node db8a0273.js update A [--silent]
  const manifest = args[1];
  ...
  console.log(JSON.stringify({
    ...
    usage: [
      'node db8a0273.js check <manifest> <subTotal> <turn>',
      'node db8a0273.js evaluate <manifest> <results.json>',
      'node db8a0273.js update <manifest>',
      'node db8a0273.js gate-reset [--silent]',
      'node db8a0273.js status',
      'node db8a0273.js reset'
    ],
    examples: [
