...
Usage:
  node 0f24f92c.js gate <tech_score> <logical_score> "<topic>" [--silent]
...
Examples:
  node 0f24f92c.js gate 90 75 "Implement user auth with JWT"
  node 0f24f92c.js gate 60 85 "Fix login bug" --silent
  node 0f24f92c.js gate 80 90 "Explain how auth works"
`);
    process.exit(0);
}
...
if (command === 'gate') {
    ...
    if (isNaN(techScore) || isNaN(logicalScore) || !topic) {
        ...
        console.error('💡 Usage: node 0f24f92c.js gate <tech_score> <logical_score> "<topic>"');
        process.exit(1);
    }
