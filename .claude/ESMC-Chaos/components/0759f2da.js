 ...
 * ════════════════════════════════════════════════════════════════
 */
/** ESMC 3.101.0 (Silent-by-default) - Base: 3.52 preference-learning CLI */
...
function showUsage() {
    console.log(`
🎖️ Preference Learning CLI - Learn and adapt to user preferences

Usage: node 0759f2da.js <command> [options]

Commands:
  learn <context>          Learn from user interactions
  analyze                  Analyze learned preferences
  help                     Show this help

Version: ESMC 3.52 | Module: db719405.js
    `);
}
...
async function main() {
    ...
    try {
        if (command === 'help' || command === '--help') { if (silent) {
            ...
            showUsage(); return; }
        const Module = require('./db719405.js');
