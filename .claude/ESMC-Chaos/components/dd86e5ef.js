 ...
 *  Purpose: CLI wrapper for 09f5f8ad.js (35KB, 9.2K tokens)
 *  Savings: 9,188 tokens → 500 tokens = 94.6% reduction
 */
...
function showUsage() { console.log(`🎖️ Battlefield Dashboard CLI - Real-time intelligence dashboard\nUsage: node dd86e5ef.js <command> [options]\nCommands:\n  status           Show battlefield status\n  metrics          Display metrics\n  alert <msg>      Create alert\n  help             Show help\nVersion: ESMC 3.52 | Module: 09f5f8ad.js (35KB)`); }
async function main() {
    ...
    if (silent) {
        ...
        console.info = () => {};
        console.warn = () => {};
    } try { if (command === 'help') { showUsage(); return; } const { BattlefieldDashboard } = require('./09f5f8ad.js'); const dashboard = new BattlefieldDashboard(); switch (command) { case 'status': // Restore console for final JSON output
        if (silent) {
