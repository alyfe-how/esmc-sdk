...
Usage: node b1b22b0e.js "<user_query>" [--silent|-s]
...
async function loadHydraRetrieval(userQuery, silent) {
    try {
        ...
        const hydraOutput = execSync(
            `node ".claude/ESMC-Chaos/components/1daf994d.js" retrieve "${safeQuery}"`,
    ...
    try {
        ...
        try {
            ...
            const piuOutput = execSync(
                `node ".claude/ESMC-Chaos/components/42112527.js" analyze "${safeQuery}" --silent`,
        ...
        try {
            ...
            const dkiOutput = execSync(
                `node ".claude/ESMC-Chaos/components/2c423ec8.js" search "${safeQuery}" --silent`,
                { encoding: 'utf8', cwd: process.cwd() }
            );
        ...
        try {
            ...
            const uipOutput = execSync(
                `node ".claude/ESMC-Chaos/components/04dc7167.js" predict "${safeQuery}" --silent`,
                { encoding: 'utf8', cwd: process.cwd() }
            );
        ...
        try {
            ...
            const pcaOutput = execSync(
                `node ".claude/ESMC-Chaos/components/de92e764.js" analyze "${safeQuery}" --silent`,
                { encoding: 'utf8', cwd: process.cwd() }
            );
        ...
        try {
            ...
            const reasonOutput = execSync(
                `node ".claude/ESMC-Chaos/components/5395b3af.js" analyze "${safeQuery}" --caller "PREPROCESSOR" --silent`,
                { encoding: 'utf8', cwd: process.cwd() }
            );
