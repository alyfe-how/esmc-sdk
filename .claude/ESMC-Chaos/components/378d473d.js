 ...
 *   - Context accuracy (full mesh + REASON) → Pattern familiarity → Zero iteration
 *   - Redesign loop DISABLED per user directive (focus on first-attempt accuracy)
 *
 * ESMC 4.1 Learning Loop:
 *   - Error-to-Lesson: IC/UID thresholds → Auto-create lesson in .esmc-lessons.json
 *   - AESD-to-Precedent: Working solutions → Add to .l5-precedents.json
 *   - Closed learning loop: Detect → Learn → Prevent future errors
 *
 * Commands:
 ...
 * Closes the learning loop: Error detected → Lesson created → Future prevention
 */
 ...
 * Closes the loop: Working solution → Becomes reusable pattern
 */
...
async function executeVetting(options = {}) {
  ...
  try {
    if (!silent) console.log('🛡️ ATHENA: Initializing error detection...');

    const EpsilonATHENACoordinator = require('./6221cade.js');
    const athena = new EpsilonATHENACoordinator();
    await athena.initialize();

    if (!silent) console.log('🛡️ ATHENA: Running PHC aggregation...');
    ...
    if (!silent) {
      console.log('🛡️ ATHENA: Vetting complete');
    ...
    if (!silent) {
      console.error('❌ ATHENA Vetting Error:', error.message);
...
async function executeReview(proposalJson, options = {}) {
  ...
  try {
    ...
    if (!silent) console.log('🛡️ ATHENA: Reviewing ECHELON proposal...');
    ...
    if (!silent) console.log('🛡️ ATHENA: Checking error signatures...');
    ...
    if (!silent) console.log('🛡️ ATHENA: Checking iteration count...');
    ...
    if (!silent) console.log('🛡️ ATHENA: Searching for precedents...');
    ...
    if (!silent) console.log('🛡️ ATHENA: Checking correction patterns...');
    ...
    if (icResult.iterationCount >= 3 || uidResult.interventionCount >= 3) {
      ...
      if (lessonResult.created) {
        ...
        if (!silent) {
          console.log(`🧠 ATHENA Learning: Auto-created ${lessonResult.lesson_id} (IC=${icResult.iterationCount}, UID=${uidResult.interventionCount})`);
        }
      }
    }
      ...
      for (const precedent of reviewResult.feedback.precedents_to_apply) {
        if (precedent.solution || precedent.solution_summary || precedent.root_cause) {
          ...
          if (addResult.added) {
            ...
            if (!silent) {
              console.log(`🧠 ATHENA Learning: Added solution from ${precedent.session_id || `Rank ${precedent.rank}`} to L5 precedents`);
            }
          }
        }
      }
    ...
    // Strategy: Context accuracy → Pattern familiarity → Zero iteration
    //
    ...
    if (!silent) {
      console.log('🛡️ ATHENA: Review complete');
    ...
    if (!silent) {
      console.error('❌ ATHENA Review Error:', error.message);
      console.log(JSON.stringify(errorResult, null, 2));
    }
...
Usage:
  node 378d473d.js vet [--silent]
  node 378d473d.js review <proposal_json> [--silent] [--iteration <num>]
...
Examples:
  node 378d473d.js vet --silent
  node 378d473d.js review '{"topic":"Add auth"}' --iteration 2
`);
  process.exit(0);
}
...
(async () => {
  try {
    if (command === 'vet') {
      ...
      if (!proposalJson) {
        console.error('❌ Missing required argument: <proposal_json>');
        process.exit(1);
      }
